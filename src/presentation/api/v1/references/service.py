...
class CustomerService:
    ...
    async def create(self, name: str, default_address: str, contact: str = "", comment: str = "") -> int:
        return await create_customer(CreateCustomerDTO(name, default_address, contact, comment), self._repo)

    async def update(self, id: int, name: str, default_address: str, contact: str = "", comment: str = "") -> None:
        await update_customer(id, UpdateCustomerDTO(name, default_address, contact, comment), self._repo)
...
class RawMaterialService:
    ...
    async def create(self, name: str, unit: str, shelf_life_days: int, critical_stock: Decimal, comment: str = "") -> int:
        return await create_raw_material(
            CreateRawMaterialDTO(name, unit, shelf_life_days, critical_stock, comment), self._repo
        )
    ...
    async def update(
        ...
        await update_raw_material(
            id, UpdateRawMaterialDTO(name, unit, shelf_life_days, critical_stock, comment), self._repo
        )
...
class PackagingService:
    ...
    async def create(self, name: str, unit: str, critical_stock: int, comment: str = "") -> int:
        return await create_packaging(CreatePackagingDTO(name, unit, critical_stock, comment), self._repo)

    async def update(self, id: int, name: str, unit: str, critical_stock: int, comment: str = "") -> None:
        await update_packaging(id, UpdatePackagingDTO(name, unit, critical_stock, comment), self._repo)
