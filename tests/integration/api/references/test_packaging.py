...
BASE = "/api/v1/references/packaging"

_VALID = {"name": "Пакет 1кг", "unit": "шт", "critical_stock": 500}
...
async def test_update_returns_200_with_new_data(client: AsyncClient, saved: int) -> None:
    r = await client.patch(
        f"{BASE}/{saved}", json={**_VALID, "name": "Пакет 500г", "critical_stock": 1000}
    )
    assert r.status_code == 200
    data = r.json()
    assert data["name"] == "Пакет 500г"
    assert data["critical_stock"] == 1000
